  ...
  const body = rows
    .map(
      ...
      <tr>
        ...
        <td>${row.url ? `<a href="${escapeHtml(row.url)}" target="_blank" rel="noopener noreferrer">Consulter</a>` : "-"}</td>
      </tr>
    `
    )
    .join("");
...
export default async function handler(req, res) {
  ...
  const [boamp, urbanisme, monaco] = await Promise.all([
    fetchJson(`${host}/api/boamp-daily`),
    fetchJson(`${host}/api/urbanisme-daily`),
    fetchJson(`${host}/api/monaco-daily`)
  ]);
  ...
  const horsCible = allRows
    .filter((r) => r.verdict === "Non pertinent")
    ...
    <body>
      <div class="container">
        <section class="card">
          ...
          <p class="subtitle">
            Hôtellerie, villas de luxe, promoteurs, urbanisme, appels d’offres, déclarations préalables, permis de construire.
          </p>
          ...
          <div class="summary-grid">
            ...
            <div class="summary-box">
              <div class="label">Hors cible</div>
              <div class="value">${horsCible.length}</div>
            </div>
            <div class="summary-box">
        ...
        ${renderTable("Hors cible", horsCible.slice(0, 30), "danger")}
      </div>
    </body>
    </html>
  `;
...
export default async function handler(req, res) {
  try {
    const baseUrl = req.headers.host?.startsWith("http")
      ? req.headers.host
      : `https://${req.headers.host}`;
    ...
    return res
      .status(500)
      .send(`<h1>Erreur</h1><pre>${escapeHtml(error.message)}</pre>`);
  }
}
